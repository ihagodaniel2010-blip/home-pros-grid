-- 000_homeleadpro_schema_draft.sql
-- Draft Schema for HomeLeadPro SaaS Multiempresa
...
create table if not exists public.leads (
    id uuid primary key default gen_random_uuid(),
    organization_id uuid references public.organizations(id) on delete set null, -- Null for public distributed leads
    source text not null default 'public' check (source in ('public', 'manual')),
    full_name text not null,
    email text not null,
    phone text not null,
    phone_masked text, -- Masked phone viaTwilio Proxy
    address text,
    city text,
    state text,
    zip text not null,
    ...
    status text not null default 'New' check (status in ('New', 'Contacted', 'Estimate Sent', 'Approved', 'Closed', 'Rejected')),
    owner_notes text,
...
-- 27. reviews (Adaptation/Extension of existing reviews table structure to match multiempresa and links)
-- Note: In Fase 2, we will run an ALTER statement or recreate the table preserving data.
-- Adding fields supporting references to organizations, leads, and jobs.
...
alter table public.reviews rename column user_name to customer_name;
alter table public.reviews rename column body to comment;
