-- 003_homeleadpro_delta_from_carpentry_existing_schema_v3.sql
-- Delta Migration to safely adapt the existing Carpentry schema to the HomeLeadPro v2 design (Version 3).
...
--------------------------------------------------------------------------------
-- 🛡️ 1. CREATE MISSING TABLES (IF NOT EXISTS)
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
-- 🛡️ 1.1. ENSURE PRE-EXISTING TABLES HAVE EXPECTED COLUMNS
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
-- 🛡️ 1.2. DEFINE PUBLIC TOKEN GENERATOR FUNCTION (MOVED UP FOR UPDATES)
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
-- 🛡️ 1.3. POPULATE MISSING TOKENS FOR SECURITY ISOLATION
--------------------------------------------------------------------------------
update public.leads
...
--------------------------------------------------------------------------------
-- 🛡️ 1.4. UNIQUE INDEXES FOR PUBLIC TOKENS (FILTERED FOR IDEMPOTENCY)
--------------------------------------------------------------------------------
...
create policy "Assigned workers can view SMS threads for their jobs" on public.sms_threads for select using (exists (select 1 from public.service_jobs where service_jobs.lead_id = sms_threads.lead_id and (service_jobs.assigned_worker_id = auth.uid() or exists (select 1 from public.employee_assignments where employee_assignments.service_job_id = service_jobs.id and employee_assignments.worker_user_id = auth.uid()))));
...
create policy "Assigned workers can view/send messages in threads they can access" on public.sms_messages for all using (exists (select 1 from public.sms_threads where sms_threads.id = sms_messages.thread_id));
...
create policy "Assigned workers can view their service jobs" on public.service_jobs for select using (assigned_worker_id = auth.uid() or exists (select 1 from public.employee_assignments where employee_assignments.service_job_id = service_jobs.id and employee_assignments.worker_user_id = auth.uid()));
create policy "Assigned workers can update status of their jobs" on public.service_jobs for update using (assigned_worker_id = auth.uid() or exists (select 1 from public.employee_assignments where employee_assignments.service_job_id = service_jobs.id and employee_assignments.worker_user_id = auth.uid())) with check (status in ('in_progress', 'completed'));
...
create policy "Assigned workers can view checklists for their jobs" on public.service_checklists for select using (exists (select 1 from public.service_jobs where service_job_id = service_checklists.service_job_id));
...
create policy "Assigned workers can view checklist tasks" on public.checklist_tasks for select using (exists (select 1 from public.service_checklists where id = checklist_tasks.checklist_id));
create policy "Assigned workers can toggle checklist tasks" on public.checklist_tasks for update using (exists (select 1 from public.service_checklists where id = checklist_tasks.checklist_id)) with check (is_completed in (true, false));
...
create policy "Assigned workers can view service extras" on public.service_extras for select using (exists (select 1 from public.service_jobs where id = service_extras.service_job_id));
...
create policy "Assigned workers can view service files for their jobs" on public.service_files for select using (exists (select 1 from public.service_jobs where id = service_files.service_job_id));
create policy "Assigned workers can insert service files for their jobs" on public.service_files for insert with check (exists (select 1 from public.service_jobs where id = service_files.service_job_id));
